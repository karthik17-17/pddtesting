...
class HtmlReportGenerator {
    static outDir = path.join(__dirname, '..', 'reports', 'HTML');
    static initialize() {
        if (!fs.existsSync(HtmlReportGenerator.outDir)) {
            fs.mkdirSync(HtmlReportGenerator.outDir, { recursive: true });
        }
    }
    static generate(testCases, deviceName, androidVersion, appVersion, totalDurationSec) {
        HtmlReportGenerator.initialize();
        logger_1.Logger.info('Generating HTML automation reports...');
        ...
        logger_1.Logger.info(`HTML Reports successfully written to: ${HtmlReportGenerator.outDir}`);
    }
    static getStyles() {
        return `
      :root {
    ...
    static buildDashboard(testCases, total, passed, failed, skipped, blocked, passRate, device, osVer, appVer, duration, commonStyles) {
    ...
    static buildExecutionReport(testCases, total, passed, failed, skipped, blocked, passRate, device, osVer, appVer, duration, commonStyles) {
    ...
    static buildTrends(total, passed, failed, skipped, passRate, commonStyles) {
