...
class ExcelReportGenerator {
    static outDir = path.join(__dirname, '..', 'reports', 'Excel');
    static initialize() {
        if (!fs.existsSync(ExcelReportGenerator.outDir)) {
            fs.mkdirSync(ExcelReportGenerator.outDir, { recursive: true });
        }
    }
    static async generate(testCases, deviceName, androidVersion, appVersion, totalDurationSec) {
        ExcelReportGenerator.initialize();
        logger_1.Logger.info('Generating Excel automation reports...');
        ...
        logger_1.Logger.info(`Master Excel report written to: ${masterPath}`);
        ...
        logger_1.Logger.info('Excel report generation completed successfully!');
    }
    static async buildMasterReport(wb, all, passed, failed, skipped, blocked, passRate, failRate, device, osVer, appVer, duration) {
        // Styling constants
        const borderStyle = {
        ...
        const darkPurpleFill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4F46E5' } };
    ...
    static buildSingleStatusSheet(wb, sheetName, tests, headerHex) {
    ...
    static buildSummaryReport(wb, total, passed, failed, skipped, blocked, passRate, failRate, device, osVer, appVer, duration) {
    ...
    static styleSheetHeadersAndBorders(ws, headerFill, font, border) {
        // Style headers
